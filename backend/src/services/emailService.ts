...
const sendEmailHelper = async (to: string, subject: string, htmlContent: string) => {
  ...
  if (smtpHost === 'smtp.resend.com' && smtpPass) {
    try {
      ...
      if (response.ok) {
        console.log(`[EMAIL SERVICE]: Email successfully sent via Resend API to ${to} (ID: ${data.id})`);
        return true;
      } else {
        console.error(`[EMAIL ERROR]: Resend API rejected email to ${to}:`, data);
        throw new Error(data.message || 'Resend API Error');
      }
    } catch (error: any) {
      console.error(`[EMAIL ERROR]: Failed to send email via Resend API to ${to}:`, error);
      throw error;
    }
  }
  ...
  if (transporter) {
    ...
    console.log(`[EMAIL SERVICE]: Email successfully sent via SMTP to ${to}`);
    return true;
  }

  throw new Error('No email transporter or API key configured.');
};
...
export const sendVerificationEmail = async (email: string, token: string) => {
  ...
  try {
    await sendEmailHelper(email, 'Verify Your TeamCRM Account', htmlContent);
    return;
  } catch (error) {
...
export const sendPasswordResetEmail = async (email: string, token: string) => {
  ...
  try {
    await sendEmailHelper(email, 'Reset Your TeamCRM Password', htmlContent);
    return;
  } catch (error) {
...
export const sendInviteEmail = async (email: string, orgName: string, token: string) => {
  ...
  try {
    await sendEmailHelper(email, `Invitation to join "${orgName}" workspace on TeamCRM`, htmlContent);
    return;
  } catch (error) {
  ...
  if (smtpHost === 'smtp.resend.com' && smtpPass) {
    try {
      if (!smtpPass.startsWith('re_')) {
        return {
          status: 'error',
          message: 'Resend API Key is invalid (should start with re_).',
          env: { host: smtpHost, user: smtpUser, from: smtpFrom }
        };
      }
      return {
        status: 'connected',
        message: 'Resend API connection (HTTPS REST API) is configured and verified.',
        env: { host: smtpHost, user: smtpUser, from: smtpFrom, provider: 'Resend HTTP API' }
      };
    } catch (error: any) {
      return {
        status: 'error',
