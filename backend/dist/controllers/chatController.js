...
exports.markAsRead = exports.addReaction = exports.getMessages = void 0;
...
const addReaction = async (req, res) => {
    try {
        ...
        if (!req.user)
            return res.status(401).json({ error: 'Unauthorized' });
        const message = await ChatMessage_1.default.findOne({ _id: messageId, orgId: req.user.orgId });
        if (!message) {
            return res.status(404).json({ error: 'Message not found' });
        }
        // Check if user already reacted with this exact emoji
        const existingReactionIndex = message.emojiReactions.findIndex((r) => r.userId.toString() === req.user?.id && r.emoji === emoji);
...
const markAsRead = async (req, res) => {
    try {
        ...
        if (!req.user)
            return res.status(401).json({ error: 'Unauthorized' });
        const userIdObj = new mongoose_1.default.Types.ObjectId(req.user.id);
